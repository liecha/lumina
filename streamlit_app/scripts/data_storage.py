# Updated data_storage.py for Electron environment
...
def get_app_directory():
    """Get the correct app directory whether running in development or packaged"""
    if getattr(sys, 'frozen', False):
        # Running in PyInstaller bundle (packaged app)
        return os.path.dirname(sys.executable)
    else:
        # Running in development
        # Check for environment variables set by Electron
        streamlit_app_dir = os.environ.get('STREAMLIT_APP_DIR')
        if streamlit_app_dir and os.path.exists(streamlit_app_dir):
            return streamlit_app_dir
        else:
...
def get_data_directory():
    ...
    # Check for environment variable set by Electron
    data_dir = os.environ.get('STREAMLIT_DATA_DIR')
    if data_dir and os.path.exists(data_dir):
        return data_dir
    
    # Fallback to app directory + data
    ...
    if not os.path.exists(data_dir):
        try:
            ...
            import tempfile
            data_dir = os.path.join(tempfile.gettempdir(), 'lumina_data')
            os.makedirs(data_dir, exist_ok=True)
            print(f"Using temporary data directory: {data_dir}")
    
    return data_dir

def resolve_csv_path(relative_path):
    """Resolve relative CSV path to absolute path"""
    # Remove 'data/' prefix if present
    filename = relative_path.replace('data/', '').replace('data\\', '')
    
    # Get absolute path in data directory
...
def fetch_from_csv(path_to_df_to_fetch):
    """Fetch data from CSV file with proper path resolution"""
    absolute_path = resolve_csv_path(path_to_df_to_fetch)
    
    try:
        # Check if file exists
        if not os.path.exists(absolute_path):
            print(f"CSV file not found: {absolute_path}")
            # Create empty file with headers
            create_empty_csv_file(absolute_path, path_to_df_to_fetch)
        
        df_fetched = pd.read_csv(absolute_path)
        print(f"Successfully loaded CSV: {absolute_path} ({len(df_fetched)} rows)")
        return df_fetched
        
    except Exception as e:
        print(f"Error reading CSV {absolute_path}: {e}")
...
def create_empty_csv_file(file_path, original_path):
    ...
    try:
        ...
        if "energy" in original_path or "database-results" in original_path:
            headers = 'date,time,label,activity,distance,energy,pro,carb,fat,note,energy_acc,protein_acc,duration,pace,steps'
        elif "livsmedelsdatabas" in original_path:
            headers = 'livsmedel,calorie,protein,carb,fat'
        elif "recipie" in original_path or "meal" in original_path:
...
def save_to_csv(df_to_store, path):
    """Save dataframe to CSV file with proper path resolution"""
    absolute_path = resolve_csv_path(path)
    ...
    try:
        ...
        # Save the dataframe
        df_to_store.to_csv(absolute_path, index=False, encoding='utf-8')
        print(f'Data was saved to CSV: {absolute_path} ({len(df_to_store)} rows)')
        
    except Exception as e:
        print(f'Error saving CSV {absolute_path}: {e}')
...
# ===================== EXISTING FUNCTIONS (unchanged but with better error handling) =====================
